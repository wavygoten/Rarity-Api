...
const _ = {
	...
	async fetchUserData(address: string) {
		return await fetch(`https://api.opensea.io/api/v2/accounts/${address}`, {
			method: "GET",
			headers: {
	...
	async fetchCollection(slug: string) {
		return await fetch(`https://api.opensea.io/api/v2/collections/${slug}`, {
			method: "GET",
			headers: {
				Accept: "application/json",
				"X-API-KEY": apiKey,
	 ...
	 * @return {Promise} Response data in JSON.
	 */
	async fetchTraits(slug: string) {
		return await fetch(`https://api.opensea.io/api/v2/traits/${slug}`, {
			method: "GET",
			headers: {
				Accept: "application/json",
				"X-API-KEY": apiKey,
	...
	async fetchAssets(contract: string, supply: number, traitRarities: any) {
		...
		if (data.contract.indexOf(contract) === -1) {
			for (let i = 0; i < supply; i++) {
				let score: number = 0;
				await fetch(
					`https://api.opensea.io/api/v2/chain/ethereum/contract/${contract}/nfts/${i}`,
					{
						method: "GET",
						headers: {
							Accept: "application/json",
					...
					.then((res: any) => {
						...
						for (let i: number = 0; i < res?.nft?.traits.length; i++) {
							for (let j: number = 0; j < traitRarities.length; j++) {
								if (
									res?.nft?.traits[i]?.value.toLowerCase() ===
						...
						result.push({
							name: res?.nft?.name,
							tokenid: res?.nft?.identifier,
							contract: contract,
							traits: res?.nft?.traits,
							opensea: res?.nft?.opensea_url,
	...
	async fetchMultipleAssets(
		...
		if (!find?.data?.data) {
			for (let i = 0; i < supply; i += 200) {
				...
				await fetch(
					`https://api.opensea.io/api/v2/chain/ethereum/contract/${contract}/nfts${params}`,
					{
						method: "GET",
						headers: {
							Accept: "application/json",
	...
	async reFetchMultipleAssets(
		...
		if (!find?.data?.data?.success) {
			for (let i = 0; i < supply; i += 30) {
				...
				await fetch(`https://api.opensea.io/api/v2/assets?${params}`, {
					method: "GET",
					headers: {
						Accept: "application/json",
						"X-API-KEY": apiKey,
	...
	/**
	 * Query data for specific token id
	 * @return {Promise} Indexed Query of the Data.
	 */
